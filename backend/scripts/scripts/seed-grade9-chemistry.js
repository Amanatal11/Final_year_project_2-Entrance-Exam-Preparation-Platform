/**
 * Seed Grade 9 Chemistry (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade9-chemistry.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade9-chemistry.js --reset   # remove existing Grade 9 Chemistry Natural + re-seed
 *
...
const curriculum = require('./data/grade9ChemistryCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade9ChemistrySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade9ChemistryExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade9ChemistryNatural() {
  ...
  console.log('Removed existing Grade 9 Chemistry (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade9ChemistryNatural();
  else {
  ...
  console.log('\n✅ Grade 9 Chemistry seed complete');
