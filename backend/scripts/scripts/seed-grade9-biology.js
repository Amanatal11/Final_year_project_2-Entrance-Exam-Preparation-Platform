/**
 * Seed Grade 9 Biology (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade9-biology.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade9-biology.js --reset   # remove existing Grade 9 Biology Natural + re-seed
 *
...
const curriculum = require('./data/grade9BiologyCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade9BiologySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade9BiologyExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade9BiologyNatural() {
  ...
  console.log('Removed existing Grade 9 Biology (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade9BiologyNatural();
  else {
