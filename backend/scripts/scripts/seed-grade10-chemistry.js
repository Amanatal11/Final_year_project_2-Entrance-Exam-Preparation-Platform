/**
 * Seed Grade 10 Chemistry (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade10-chemistry.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade10-chemistry.js --reset   # remove existing Grade 10 Chemistry Natural + re-seed
 *
...
const curriculum = require('./data/grade10ChemistryCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade10ChemistrySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade10ChemistryExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade10ChemistryNatural() {
  ...
  console.log('Removed existing Grade 10 Chemistry (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade10ChemistryNatural();
  else {
  ...
  console.log('\n✅ Grade 10 Chemistry seed complete');
