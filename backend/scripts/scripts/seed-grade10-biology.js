/**
 * Seed Grade 10 Biology (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade10-biology.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade10-biology.js --reset   # remove existing Grade 10 Biology Natural + re-seed
 *
...
const curriculum = require('./data/grade10BiologyCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade10BiologySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade10BiologyExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade10BiologyNatural() {
  ...
  console.log('Removed existing Grade 10 Biology (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade10BiologyNatural();
  else {
  ...
  console.log('\n✅ Grade 10 Biology seed complete');
