/**
 * Seed Grade 11 Biology (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade11-biology.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade11-biology.js --reset   # remove existing Grade 11 Biology Natural + re-seed
 *
...
const curriculum = require('./data/grade11BiologyCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade11BiologySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade11BiologyExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade11BiologyNatural() {
  ...
  console.log('Removed existing Grade 11 Biology (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade11BiologyNatural();
  else {
  ...
  console.log('\n✅ Grade 11 Biology seed complete');
