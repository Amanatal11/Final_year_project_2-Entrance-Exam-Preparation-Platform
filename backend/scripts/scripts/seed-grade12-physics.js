/**
 * Seed Grade 12 Physics (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade12-physics.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade12-physics.js --reset   # remove existing Grade 12 Physics Natural + re-seed
 *
...
const curriculum = require('./data/grade12PhysicsCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade12PhysicsSeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade12PhysicsExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade12PhysicsNatural() {
  ...
  console.log('Removed existing Grade 12 Physics (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade12PhysicsNatural();
  else {
  ...
  console.log('\n✅ Grade 12 Physics seed complete');
