/**
 * Seed Grade 11 Physics (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade11-physics.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade11-physics.js --reset   # remove existing Grade 11 Physics Natural + re-seed
 *
...
const curriculum = require('./data/grade11PhysicsCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade11PhysicsSeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade11PhysicsExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade11PhysicsNatural() {
  ...
  console.log('Removed existing Grade 11 Physics (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade11PhysicsNatural();
  else {
  ...
  console.log('\n✅ Grade 11 Physics seed complete');
