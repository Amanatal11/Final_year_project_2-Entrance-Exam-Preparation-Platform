/**
 * Seed Grade 11 Chemistry (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade11-chemistry.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade11-chemistry.js --reset   # remove existing Grade 11 Chemistry Natural + re-seed
 *
...
const curriculum = require('./data/grade11ChemistryCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade11ChemistrySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade11ChemistryExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade11ChemistryNatural() {
  ...
  console.log('Removed existing Grade 11 Chemistry (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade11ChemistryNatural();
  else {
  ...
  for (let chIdx = 0; chIdx < curriculum.chapters.length; chIdx += 1) {
    ...
    const topicDocs = [];
    for (let tIdx = 0; tIdx < ch.topics.length; tIdx += 1) {
  ...
  console.log('\n✅ Grade 11 Chemistry seed complete');
  ...
  console.log(`   Exam questions (5 per topic): ${examQuestionCount}`);
}
