/**
 * Seed Grade 10 Physics (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade10-physics.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade10-physics.js --reset   # remove existing Grade 10 Physics Natural + re-seed
 *
...
const curriculum = require('./data/grade10PhysicsCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade10PhysicsSeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade10PhysicsExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade10PhysicsNatural() {
  ...
  console.log('Removed existing Grade 10 Physics (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade10PhysicsNatural();
  else {
  ...
  console.log('\n✅ Grade 10 Physics seed complete');
