/**
 * Seed Grade 12 Chemistry (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade12-chemistry.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade12-chemistry.js --reset   # remove existing Grade 12 Chemistry Natural + re-seed
 *
...
const curriculum = require('./data/grade12ChemistryCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade12ChemistrySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade12ChemistryExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade12ChemistryNatural() {
  ...
  console.log('Removed existing Grade 12 Chemistry (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade12ChemistryNatural();
  else {
  ...
  console.log('\n✅ Grade 12 Chemistry seed complete');
