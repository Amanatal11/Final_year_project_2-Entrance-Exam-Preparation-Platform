/**
 * Seed Grade 9 Physics (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade9-physics.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade9-physics.js --reset   # remove existing Grade 9 Physics Natural + re-seed
 *
...
const curriculum = require('./data/grade9PhysicsCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade9PhysicsSeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade9PhysicsExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade9PhysicsNatural() {
  ...
  console.log('Removed existing Grade 9 Physics (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade9PhysicsNatural();
  else {
