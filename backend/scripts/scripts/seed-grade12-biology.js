/**
 * Seed Grade 12 Biology (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade12-biology.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade12-biology.js --reset   # remove existing Grade 12 Biology Natural + re-seed
 *
...
const curriculum = require('./data/grade12BiologyCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade12BiologySeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade12BiologyExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade12BiologyNatural() {
  ...
  console.log('Removed existing Grade 12 Biology (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade12BiologyNatural();
  else {
  ...
  console.log('\n✅ Grade 12 Biology seed complete');
